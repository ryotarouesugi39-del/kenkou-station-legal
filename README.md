...
| `/apps/kenkou-station/terms` | 健康ステーションの利用規約 |
...
`/apps/<slug>` 以下に概要・terms・privacy・supportの4ページが増える構成です
（下記「新しいアプリを追加する手順」参照）。
...
2. `app/apps/kenkou-station/` フォルダをコピーして
   `app/apps/<新しいslug>/` を作る（`page.tsx`, `terms/page.tsx`,
   `privacy/page.tsx`, `support/page.tsx` の4ファイル）
3. コピーしたページ内の `getApp("kenkou-station")` を
   `getApp("<新しいslug>")` に書き換える
4. 利用規約・プライバシーポリシー本文（サービス内容・取得する情報・
   利用目的など）を、そのアプリの実態に合わせて書き換える
